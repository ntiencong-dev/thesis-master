...
Scenarios
---------
CI-01  Qdrant reachable  → _qdrant_client is set
CI-02  Qdrant unreachable → raises RuntimeError (no Faiss fallback)
...
CI-07  FileNotFoundError → mark_dead (no retry)
CI-08  purge_missing_files → stale pending jobs marked dead
CI-09  stop() does not join a persist thread (persist thread removed in v3.0)
"""
...
def test_CI01_qdrant_backend_sets_client_when_reachable():
    """When Qdrant is reachable, _qdrant_client must be set."""
...
# ---------------------------------------------------------------------------
# CI-02  Qdrant unreachable → raises RuntimeError
# ---------------------------------------------------------------------------

@pytest.mark.unit
def test_CI02_qdrant_unreachable_raises_runtimeerror():
    """When Qdrant is unreachable, _init_vector_store() must raise RuntimeError."""
    with patch("src.continuous_indexer.create_engine", return_value=_make_mock_engine()), \
         patch("src.continuous_indexer.PersistentJobQueue"), \
...
# ---------------------------------------------------------------------------
# CI-09  stop() does not have a persist thread (removed in v3.0)
# ---------------------------------------------------------------------------

@pytest.mark.unit
def test_CI09_stop_has_no_persist_thread():
    """ContinuousIndexer must NOT have _thread_persist attribute (removed in v3.0)."""
    mock_client = MagicMock()
    mock_client.get_collections.return_value.collections = []
    ...
    assert not hasattr(ci, "_thread_persist"), \
        "_thread_persist must not exist in v3.0 (persist loop removed)"
